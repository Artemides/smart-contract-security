...
contract VaultShares is ERC4626, IVaultShares, AaveAdapter, UniswapAdapter, ReentrancyGuard {
    //us is the max per token or whole vault limit? ok => vault's asset holdings
    ...
    //us is i_vaultGuardians DAO?, ok => no, vaults belong to a VaultGuardian Contract
    ...
    /**
     * @notice removes all supplied liquidity from Uniswap and supplied lending amount from Aave and then re-invests it back into them only if the
     * vault is active
     */
        ...
        UniswapAdapter(constructorData.uniswapRouter, constructorData.weth, constructorData.usdc)
    {
        ...
        // External calls
        i_aaveAToken = IERC20(IPool(constructorData.aavePool).getReserveData(address(constructorData.asset)).aTokenAddress);
     ...
     */
    //us does quitting divests before? ok => depens on  how is used, but protocol sets NotActive on quiting which divests and invests guardian's
    // holdings
    ...
    function updateHoldingAllocation(AllocationData memory tokenAllocationData) public onlyVaultGuardians isActive {
        uint256 totalAllocation = tokenAllocationData.holdAllocation + tokenAllocationData.uniswapAllocation + tokenAllocationData.aaveAllocation;
     ...
     */
    // slither-disable-start reentrancy-eth
    function deposit(uint256 assets, address receiver) public override(ERC4626, IERC4626) isActive nonReentrant returns (uint256) {
    ...
    /**
     * @notice Invests user deposited assets into the investable universe (hold, Uniswap, or Aave) based on the allocation data set by the vault
     * guardian
     * @param assets The amount of assets to invest
     */
    //@us? investor of 100 tokens receive same earning as 1M tokens investors?
    //@Proof: No proportional earnings for investors
     ...
     */
    //us? consequent withdraws makes divestAndInvest within short instants, potentially causing slippages over expected funds to withdraw
    //proof
    function withdraw(
        uint256 assets,
        address receiver,

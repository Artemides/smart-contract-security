...
library MathMasters {
    /*//////////////////////////////////////////////////////////////
                                 ERRORS
    //////////////////////////////////////////////////////////////*/
    //@audit 0.8.3 does not have custom errors.
    ...
    // History lesson: WAD, RAY, and RAD were introduced in DappHub/DappTools/the DS test system and popularized by MakerDAO's original DAI system. The
    // names sort of stuck.
    ...
    function mulWad(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            if mul(y, gt(x, div(not(0), y))) {
                ...
                //@audit wrong error selector
                //@audit do not override free memory pointer
                revert(0x1c, 0x04)
            }
            z := div(mul(x, y), WAD)
        }
    ...
    function mulWadUp(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            //@audit works for  x <= y < 2x only
            //@audit unnecessary
            if iszero(sub(div(add(z, x), y), 1)) { x := add(x, 1) }
            //@audit for numbers with less digits than WAD -> 0
            //x * y >= WAD

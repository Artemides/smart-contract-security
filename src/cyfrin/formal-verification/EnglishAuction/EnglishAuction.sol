...
contract EnglishAuction {
    ...
    constructor(address _nft, address _erc20, uint256 _nftId, uint256 _startingBid) {
        nft = ERC721Mock(_nft);
        nftId = _nftId;
    ...
    function bidFor(address bidder, uint256 amount) external {
        _bid(bidder, msg.sender, amount);
    }

    /// Bidding implementation.
    /// @dev Funds are transferred from `payer` to support the bid of `bidder`.
    function _bid(address bidder, address payer, uint256 amount) internal {
    ...
    function withdrawAmount(address recipient, uint256 amount) external {
    ...
    function withdrawFor(address bidder, uint256 amount) external {
        require(operators[bidder][msg.sender] || msg.sender == bidder, "that operator was not allowed");

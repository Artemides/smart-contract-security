...
contract TwentyOne {
    ...
    function drawCard(address player) internal returns (uint256) {
        ...
        uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.prevrandao))) % availableCards[player].length;
    ...
    function _canPayout() internal view returns (bool) {
        return address(this).balance >= (players + 1) * 2;
    }
}

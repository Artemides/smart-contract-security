...
contract MDex {
    ...
    function drainReserves() external {
        ...
        while (token0.balanceOf(address(dex)) > 0 && token1.balanceOf(address(dex)) > 0) {
            _swap(token0, token1);
            _swap(token1, token0);
        }
    }
    ...
    function _swap(IERC20 token0, IERC20 token1) internal {
        ...
        uint256 swapAmount1 = dex.getSwapPrice(address(token0), address(token1), anyone0);
    ...
    function _swapAllReserves(IERC20 token0, IERC20 token1, uint256 reserves1) internal {
    ...
    function _getAmountIn(IERC20 token0, IERC20 token1, uint256 amountOut) internal view returns (uint256) {
        uint256 reserves0 = token0.balanceOf(address(dex));
        uint256 reserves1 = token1.balanceOf(address(dex));
